...
#define USART_RX_PORT                   (GPIO_PORT_B)   /* PB13: USART4_RX */
#define USART_RX_PIN                    (GPIO_PIN_13)
#define USART_RX_GPIO_FUNC              (GPIO_FUNC_33)

#define USART_TX_PORT                   (GPIO_PORT_B)   /* PB12: USART4_TX */
...
modbus_st uart0;
SemaphoreHandle_t semCounting_Usart0recv;
SemaphoreHandle_t semCounting_Usart0send;
int32_t drv_uart0_PRINTF_Preinit(void *vpDevice, uint32_t u32Baudrate);
...
static void USART_RxFull_IrqCallback(void)
{
	...
	if(uart0.rece_ptr < MODBUS_BUF_SIZE)
	{
		uart0.rece_buf[uart0.rece_ptr++] = u16Data;
		xSemaphoreGiveFromISR(semCounting_Usart0recv,&semRet);
	}
...
{	
	if (uart0.send_ptr < uart0.send_bytes)
	{
		USART_WriteData(USART_UNIT, (uint16_t)uart0.send_buf[uart0.send_ptr++]);
	}
	else
	{
...
static void USART_TxComplete_IrqCallback(void)
{
	...
	xSemaphoreGiveFromISR(semCounting_Usart0send,&semRet);
}
...
void drv_uart0_init(struct modbus* this)
{
	...
	if(semCounting_Usart0recv == NULL)
	{
		semCounting_Usart0recv = xSemaphoreCreateCounting(1,0);   //最大1   当前0
	}
	if(semCounting_Usart0send == NULL)
	{
		semCounting_Usart0send = xSemaphoreCreateCounting(1,0);   //最大1   当前0
	}

	if(SysParaKeep.uart0_config == Printf_uart_DISENABLE
	|| SysParaKeep.uart0_config == Printf_uart_ERROR
	|| SysParaKeep.uart0_config == Printf_uart_INFO
	|| SysParaKeep.uart0_config == Printf_uart_DEBUG
	){
		DDL_PrintfInit(USART_UNIT, u32Baudrate, drv_uart0_PRINTF_Preinit);
	}
}
int32_t drv_uart0_PRINTF_Preinit(void *vpDevice, uint32_t u32Baudrate)
{
...
uint8_t drv_uart0_read(master_slave_mode mode, struct modbus* this)
{
	...
	{
		if (xSemaphoreTake(semCounting_Usart0recv,(TickType_t)noDataTimeout) == pdFALSE
		&& this->rece_ptr >= 5)
		{
...
uint8_t drv_uart0_write(struct modbus* this)
{
	...
	if(xSemaphoreTake(semCounting_Usart0send,(TickType_t)1000) == pdTRUE)
	{
		return OK;
	}
