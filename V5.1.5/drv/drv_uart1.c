...
#define USART_RX_PORT                   (GPIO_PORT_E)   /* PE10: USART4_RX */
#define USART_RX_PIN                    (GPIO_PIN_10)
#define USART_RX_GPIO_FUNC              (GPIO_FUNC_33)

#define USART_TX_PORT                   (GPIO_PORT_E)   /* PE11: USART4_TX */
...
#define USART_RX_ERR_IRQn               (INT008_IRQn)
#define USART_RX_ERR_INT_SRC            (INT_SRC_USART3_EI)

#define USART_RX_FULL_IRQn              (INT009_IRQn)
#define USART_RX_FULL_INT_SRC           (INT_SRC_USART3_RI)

#define USART_TX_EMPTY_IRQn             (INT010_IRQn)
#define USART_TX_EMPTY_INT_SRC          (INT_SRC_USART3_TI)

#define USART_TX_CPLT_IRQn              (INT011_IRQn)
#define USART_TX_CPLT_INT_SRC           (INT_SRC_USART3_TCI)
		
#define RS485_EN_PORT					(GPIO_PORT_E)  
#define RS485_EN_PIN					(GPIO_PIN_09)

#define RS485_EN_TX()					GPIO_SetPins(RS485_EN_PORT, RS485_EN_PIN)
#define RS485_EN_RX()					GPIO_ResetPins(RS485_EN_PORT, RS485_EN_PIN)

modbus_st uart1;
SemaphoreHandle_t semCounting_Usart1recv;
SemaphoreHandle_t semCounting_Usart1send;
int32_t drv_uart1_PRINTF_Preinit(void *vpDevice, uint32_t u32Baudrate);
...
static void USART_RxFull_IrqCallback(void)
{
	...
	if(uart1.rece_ptr < MODBUS_BUF_SIZE)
	{
		uart1.rece_buf[uart1.rece_ptr++] = u16Data;
		xSemaphoreGiveFromISR(semCounting_Usart1recv,&semRet);
	}
...
{	
	if (uart1.send_ptr < uart1.send_bytes)
	{
		USART_WriteData(USART_UNIT, (uint16_t)uart1.send_buf[uart1.send_ptr++]);
	}
	else
	{
...
static void USART_TxComplete_IrqCallback(void)
{
	...
	xSemaphoreGiveFromISR(semCounting_Usart1send,&semRet);
}
...
void drv_uart1_init(struct modbus* this)
{
	...
	if(semCounting_Usart1recv == NULL)
	{
		semCounting_Usart1recv = xSemaphoreCreateCounting(1,0);   //最大1   当前0
	}
	if(semCounting_Usart1send == NULL)
	{
		semCounting_Usart1send = xSemaphoreCreateCounting(1,0);   //最大1   当前1
	}

	if(SysParaKeep.uart1_config == Printf_uart_DISENABLE
	|| SysParaKeep.uart1_config == Printf_uart_ERROR
	|| SysParaKeep.uart1_config == Printf_uart_INFO
	|| SysParaKeep.uart1_config == Printf_uart_DEBUG
	){
		DDL_PrintfInit(USART_UNIT, u32Baudrate, drv_uart1_PRINTF_Preinit);
	}
}
int32_t drv_uart1_PRINTF_Preinit(void *vpDevice, uint32_t u32Baudrate)
{
...
uint8_t drv_uart1_read(master_slave_mode mode, struct modbus* this)
{
	...
	{
		if (xSemaphoreTake(semCounting_Usart1recv,(TickType_t)noDataTimeout) == pdFALSE
		&& this->rece_ptr >= 5)
		{
...
uint8_t drv_uart1_write(struct modbus* this)
{
	...
	if(xSemaphoreTake(semCounting_Usart1send,(TickType_t)1000) == pdTRUE)
	{
		return OK;
	}
