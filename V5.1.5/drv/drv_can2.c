...
#define CAN_INT_IRQn                    (INT093_IRQn)	
#else
...
can_handle_st can2;
SemaphoreHandle_t semCounting_can2recv;
...
static void CAN_IrqCallback(void)
{
    ...
    if ((u32Status & CAN_FLAG_PTB_TX) != 0U) {
		...
		frame_st frame;
		if(can2.tx_queue.read(&can2.tx_queue, &frame) == 0)
		{
    ...
    if ((u32Status & CAN_FLAG_RX) != 0U) {
		...
		/* Get all received frames. */
		 while (CAN_GetRxFrame(CAN_UNIT, &stc_can_rx_frame) == LL_OK)
		 {
			...
			if(stc_can_rx_frame.u32ID == 0x18017F7F+(SysParaKeep.ExtAddr<<8))
			{
				if(SysParaVoltiage.ms_cnt-SysParaVoltiage.can_ms <= 900)//广播请求1S响应一次
					break;
				else
					SysParaVoltiage.can_ms=SysParaVoltiage.ms_cnt;
			}
			if(can2.rx_queue.write(&can2.rx_queue, &frame) == 0)
			{
				xSemaphoreGiveFromISR(semCounting_can2recv,&semRet);
			}
			else
			{
				can2.rx_queue.empty(&can2.rx_queue);
			}
		}
...
static void CanInitConfig(uint32_t baudrate)
{
	...
	if (SysParaKeep.can2_config == KeGong_bmu_can)
	{
	...
	else if(SysParaKeep.can2_config == KeGong_ems_can)
	{
	...
	// 500：500kbps
	// 1000：1000kbps
	if ((SysParaKeep.can2_baud == 0 && baudrate == 1000000)
	||  (SysParaKeep.can2_baud == 1000))
	{
	...
	else if ((SysParaKeep.can2_baud == 0 && baudrate == 500000)
		 ||  (SysParaKeep.can2_baud == 500))
	{
	...
	else if ((SysParaKeep.can2_baud == 0 && baudrate == 125000)
		 ||  (SysParaKeep.can2_baud == 125))
	{
...
void drv_can2_init(can_handle_st *this, uint32_t baudrate)
{
	...
	if(semCounting_can2recv == NULL)
	{
		semCounting_can2recv = xSemaphoreCreateCounting(255,0);  //最大255 当前0
	}
}
...
uint8_t drv_can2_receive(can_handle_st *this, uint32_t *id, uint8_t *data)
{
	...
	if(xSemaphoreTake(semCounting_can2recv, (TickType_t)300) == pdTRUE)
	{
		uint8_t ret_u8 = 0;
		frame_st frame;
		taskENTER_CRITICAL();
...
uint8_t drv_can2_receive_detail(can_handle_st *this, frame_st *frame, uint16_t ms)
{
	...
	if(xSemaphoreTake(semCounting_can2recv, (TickType_t)ms) == pdTRUE)
	{
		uint8_t ret_u8 = 0;
		taskENTER_CRITICAL();
		ret_u8 = this->rx_queue.read(&this->rx_queue, frame);
