...
/* 双分区表运行时切换：V1(400K/B002)和V2(800K/B003)都编译进固件
 * 由g_flash_layout_version(在initandset.c中检测boot区版本号设置)决定使用哪张表
 * 默认V2(B003/800K)，检测到B002时切换为V1(B002/400K)
 */
extern uint8_t g_flash_layout_version;  /* FLASH_LAYOUT_V1 或 FLASH_LAYOUT_V2，定义于fal_cfg.h */
static const struct fal_partition partition_table_def_v1[] = FAL_PART_TABLE_V1;
static const struct fal_partition partition_table_def_v2[] = FAL_PART_TABLE_V2;
static const struct fal_partition *partition_table = NULL;
/* partition and flash object information cache table
 * 两张表条目数相同(均为12)，cache数组大小取max(V1,V2)=12 */
#define PART_TABLE_MAX_LEN  12
static struct part_flash_info part_flash_cache[PART_TABLE_MAX_LEN] = { 0 };
...
#ifdef FAL_PART_HAS_TABLE_CFG
    /* 运行时根据boot区版本号选择分区表
     * g_flash_layout_version=FLASH_LAYOUT_V1 → V1表(400K/B002)
     * g_flash_layout_version=FLASH_LAYOUT_V2 → V2表(800K/B003，默认)
     * 两张表条目数相同，cache数组无需调整
     */
    if (g_flash_layout_version == FLASH_LAYOUT_V1)
    {
        partition_table = &partition_table_def_v1[0];
        partition_table_len = sizeof(partition_table_def_v1) / sizeof(partition_table_def_v1[0]);
    }
    else
    {
        partition_table = &partition_table_def_v2[0];
        partition_table_len = sizeof(partition_table_def_v2) / sizeof(partition_table_def_v2[0]);
    }
#else
